...
exports.store = async (req, res) => {
  try {
    ...
    story.startDate = new Date().toLocaleString("en-US", {
      timeZone: "Asia/Kolkata",
    });
    story.endDate = new Date(
      new Date().setSeconds(new Date().getSeconds() + 86400)
...
exports.delete = async (req, res) => {
  try {
    ...
    if (!story) {
      return res
        .status(200)
        .json({ status: false, message: "Story does not exist!!" });
    }
...
exports.expireStory = async (req, res) => {
  ...
  const crtDate = new Date().toLocaleString("en-US", {
    timeZone: "Asia/Kolkata",
  });
