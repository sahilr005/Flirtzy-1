...
exports.requestStore = async (req, res) => {
  try {
    if (!req.body.bio || !req.body.userId || !req.file || !req.body.country) {
      deleteFile(req.file);
      return res
        .status(200)
...
exports.requestUpdate = async (req, res) => {
  try {
    ...
    if (req.file) {
      console.log("image in host request", request.image);

      if (fs.existsSync("storage" + image[1])) {
        fs.unlinkSync("storage" + image[1]);
      }

      request.image = baseURL + req.file.path;

      console.log("image in host request after update", request.image);
    }
    ...
    // request.country = req.body.country ? req.body.country : request.country;

    await request.save();

    const request_ = await Request.findById(request._id);

    return res
      .status(200)
      .json({ status: true, message: "Success!!", request: request_ });
  } catch (error) {
    console.log(error);
    deleteFile(req.file);
    return res.status(500).json({
...
exports.requestAccept = async (req, res) => {
  try {
    ...
    if (request.isAccepted === true) {
      return res
        .status(200)
        .json({ status: false, message: "This Host Is already Accept!!!" });
    }
    ...
    return res
      .status(200)
      .json({ status: true, message: "Success!!", request });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
